...
{
	for (int i = 0; i < BLOOM_HASH_NUM; i++) { //iterate through hash functions
		//the position to be set in the bitmap
		int pos = hash_i(i, elm) % bf->bsz;
		//get char and set the (7 - pos % 8) bit
...
{
	for (int i = 0; i < BLOOM_HASH_NUM; i++) { //iterate through hash functions
		//the position to be check in the bitmap
		int pos = hash_i(i, elm) % f->bsz;
		//get char and check (7 - pos % 8) bit
		if (((f->buf[pos / 8] >> (7 - pos % 8)) & 1) != 1) {
			return false;
		}
	}
	return true;
